...
class HospitalScheduleHour(models.TransientModel):
    ...
    hour = fields.Selection(selection=[('6', '6'), ('7', '7'), ('8', '8'), ('9', '9'),
...
class HospitalScheduleDay(models.TransientModel):
    ...
    weekday = fields.Selection(selection=[('0', 'Monday'),
                                          ...
                                          ('5', 'Saturday'),
                                          ('6', 'Sunday')])
    hour_ids = fields.Many2many(comodel_name='hospital.schedule.hour', string='Hours')
...
class HospitalFillDoctorScheduleWizard(models.TransientModel):
    ...
    weekday_ids = fields.Many2many(comodel_name='hospital.schedule.day', string='Weekdays')
    ...
    def to_fill_doctor_schedule(self):
        ...
        for rec in self:
            ...
            for i in range(date_interval.days):
                ...
                if (rec.weekday_type == 'all' or (rec.weekday_type == 'even' and week_num % 2 == 0) or (rec.weekday_type == 'odd' and week_num % 2 != 0)):
            ...
            for one_day in suitable_days:
                for weekdays in rec.weekday_ids:
                    ...
                    for hours in weekdays.hour_ids:
                        if self.env['hospital.doctor.schedule'].sudo().search_count([('doctor_id', '=', rec.doctor_id.id),
                                                                                     ('date', '=', one_day),
                                                                                     ('hour', '=', int(hours.hour))]) == 0:
                            # print(rec.doctor_id.name, ' ', weekdays.weekday, ' ', hours.hour)
                            self.env['hospital.doctor.schedule'].sudo().create({'doctor_id': rec.doctor_id.id,
                                                                                'date': one_day,
                                                                                'hour': int(hours.hour)})
