...
class HospitalDoctor(models.Model):
    ...
    intern_ids = fields.One2many(comodel_name='hospital.doctor', inverse_name='mentor_id', string='Interns')
    ...
    def _constrains_mentor(self):
        for rec in self:
            if rec.mentor_id._is_intern():
                raise ValidationError(_('Intern ' + rec.mentor_id.name + ' cannot be a mentor'))
